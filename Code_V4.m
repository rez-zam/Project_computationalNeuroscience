...
imagesc(v4population(pop).spikecounts)
colorbar; 
...
num_trials = size(v4population(pop).spikecounts, 2);
num_neurons = size(v4population(pop).spikecounts, 1);
...
for i = 1:num_neurons
    firing_rates = v4population(pop).spikecounts(i, :) / duration;
    plot(firing_rates, 'b');
end
...
for i = 1:num_neurons
    spike_counts = v4population(pop).spikecounts(i, :);
